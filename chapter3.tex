...
As presented before. The performance and load indicators are stored in the RDF store and the SOS server. To gather the information from the database we have to query some custom properties. These properties are introduced in this section. 

The observable properties are the descriptors of the system property, like cpu load or free memory. A table of these can be read from Table .All custom URI starts with http://mit.bme.hu/sensorweb.
\begin{table}[h]
	\begin{center}
		\begin{tabular}{| l | p{8cm} |}
			\hline
			URI end  &  Description \\ \hline \hline
			observableProperty/node\_utilization\_cpu\_us &  CPU load for user level processes in percentage.\\ \hline
			observableProperty/node\_utilization\_cpu\_sy &  CPU load for system level processes in percentage.\\ \hline
			observableProperty/node\_utilization\_cpu\_tw &  Ratio of the idle (twiddling) process compared to systems overall performance.\\ \hline
			observableProperty/node\_utilization\_cpu\_wa &  Ratio of waiting for IO processes compared to overall performance.\\ \hline
			observableProperty/node\_utilization\_mem\_free &  Free memory in bytes.\\ \hline			
			\\TODO we have to finish this part.
		\end{tabular}
		\caption{Observable properties\label{tab:obsprop}}
	\end{center}
\end{table}
